...
# url: http://opiateforthemass.es/articles/sakura/                              #
#################################################################################
...
wd <- "D:/Data Science/Data visualization/"
setwd(wd)

# download data if not already on disk
if(!file.exists("data/sakura.xls")){
  ...
  download.file(aono.dta, "data/sakura.xls", mode= "wb")
} 

# read in data
dta <- readxl::read_excel("data/sakura.xls", skip = 25) %>% 
# make "good" column names
# base::make.names: Make syntactically valid names out of character vectors.
# WATCH: Remove SetNames create error?
...
dta %>% 
  ...
  mutate(
    ...
    # remove all the year AD and replace with 2000
